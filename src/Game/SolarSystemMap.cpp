...
void SolarSystemMap::genMap(unsigned systemRadius, unsigned hexRadius)
{
    ...
    for (int i = int(m_systemRadius)*-1; i <= int(m_systemRadius); ++i)
    {
        genHexLine(i);
    }
}
...
void SolarSystemMap::genHexLine(int lineHeight)
{
    ...
    float hexHeight = m_hexRadius * 2 + (16 * float(SIZE_FACTOR));
    ...
    hexTemplate.setOrigin(float(m_hexRadius), float(m_hexRadius));
    ...
    for (int i = 0; i < lineWidth; ++i)
    {
        ...
        coordinates coords(int(m_systemRadius)*-1 + i, lineHeight);
        ...
        hex.move(coords.x * offset.x, coords.y * offset.y * 0.75f);
